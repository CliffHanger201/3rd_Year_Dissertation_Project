...
# ---- Optional Keras (graceful degradation if not installed) ----
try:
...
# ---- Local imports ----
# Import from the rebuilt base HH.  All names are identical to the original.
from python_hyper_heuristic.src.hyperheuristic import (
    AdvancedChoiceFunctionHH,
    HHConfig,
    Phase,
    AcceptanceKind,
    HeuristicStats,
...
class HHState:
    ...
    """
    # --- summary features across all heuristics ---
    best_reward_ema:  float = 0.0   # max reward EMA across all heuristics
    worst_reward_ema: float = 0.0   # min reward EMA across all heuristics
    best_tail_score:  float = 0.0   # max tail score across all heuristics
    reward_spread:    float = 0.0   # max - min EMA (decisiveness of preference)
...
def build_state(hh: "PreTrainedHH", h_count: int) -> HHState:
    ...
    tail = np.array([hh.tail_system.score(i) for i in range(h_count)],
                    dtype=np.float32)
...
class QTable:
    ...
    def load(cls, path: str, h_count_override: Optional[int] = None) -> "QTable":
        ...
        if h_count == d["h_count"]:
            ...
            # §4: resize rows when transferring to a domain with a different
            # heuristic count — copy what fits, zero-pad the rest.
...
class EpsilonSchedule:
    ...
    def switch_to_online(self) -> None:
        """§10: called at deployment time to switch to fixed online ε."""
        self._online = True
...
class PerturbationMechanism:
    ...
    def check(self, iter_since_improve: int) -> bool:
        """Returns True when perturbation mode should activate."""
    ...
    def consume(self) -> bool:
        """Call each iteration during perturbation; returns True while active."""
        if not self._active:
            return False
        self._remaining -= 1
    ...
    def select_perturb_heuristic(
        self, h_count: int, h_stats: List[HeuristicStats],
        rng: random.Random, iteration: int,
    ) -> int:
        """Pick the least-recently-used heuristic as perturbation move."""
        staleness = [iteration - s.last_used_iter for s in h_stats]
        # Weighted random among top-50 % most stale
...
class OfflineCollector:
    ...
    def __init__(self, hh_factory, time_limit_ms: int = 5000):
        """
        hh_factory: callable() → PreTrainedHH instance
        """
    ...
    def _run_and_collect(
        self, hh: "PreTrainedHH", problem: ProblemDomain,
    ) -> List[Transition]:
        """
        Thin wrapper: install the transition hook, run, return collected data.
        PreTrainedHH exposes _transition_hook for this purpose.
        """
    ...
    def build_q_targets(self, qt: QTable, gamma: float = 0.95) -> np.ndarray:
        ...
        for i, (a, r, s_nxt, done) in enumerate(
            zip(A, R, X_nxt, [t.done for t in self.transitions])
        ):
...
class PreTrainer:
    ...
    2. Warm-start Q-Table from offline transitions (offline RL)
    ...
    def __init__(
        ...
        self.qt = QTable(h_count, state_bins=qtable_bins,
                         lr=qtable_lr, gamma=qtable_gamma)
        self.eps_schedule = EpsilonSchedule(
            eps_start=eps_start, eps_min=eps_min, decay_steps=decay_steps)
        self.tail = TailSystem(h_count, window=tail_window)

        self.surrogate_model = (
            build_surrogate_model(state_dim, h_count)
            if self.use_surrogate else None
        )
    ...
    def run_offline(
        ...
        if self.use_surrogate and self.surrogate_model is not None:
            ...
            history, test_loss = train_surrogate(
                self.surrogate_model, X, Y, epochs=self.surrogate_epochs)
    ...
    def save_qtable(self, path: str) -> None:
        """Serialise Q-Table to disk for later transfer injection."""
        self.qt.save(path)
        print(f"[PreTrainer] Q-Table saved to {path}.")

    def make_deployment_hh(
        self, config: Optional[HHConfig] = None,
    ) -> "PreTrainedHH":
        ...
        hh.eps_schedule.switch_to_online()   # §10: re-introduce small ε
        return hh
...
class PreTrainedHH(AdvancedChoiceFunctionHH):
    ...
    def __init__(
        ...
        eps_start:   float = 0.3,      # lower default for online deployment
        eps_min:     float = 0.02,
        ...
        self.qtable: Optional[QTable] = self._injected_qtable
        self.run_number = run_number

        # Logarithmic CF decay — CF dominates early, Q-Table takes over later.
        # run 1 → cf_weight ≈ 0.5 ; run 30 → cf_weight ≈ 0.29
        if HAS_KERAS:
            self.cf_weight = 0.5 / (1.0 + math.log(run_number))
            self.q_weight  = 1.0 - self.cf_weight
        else:
            self.cf_weight = 1.0 / (1.0 + math.log(run_number))
            self.q_weight  = 1.0 - self.cf_weight
        ...
        self.sliding_window:   Deque[float] = deque(maxlen=50)
    ...
    def _init_stats(self, h_count: int) -> None:
        ...
        # Initialise or resize tail system to match domain's heuristic count
        ...
        # Re-attach injected qtable in case super().__init__ wiped it
    ...
    def _select_heuristic(self, h_count: int, prev_h: Optional[int]) -> int:
        # Update ε for this step
        self.epsilon = self.eps_schedule.step()

        # Snapshot state before we pick the action (needed for Q update)
        self._pre_action_state = build_state(self, h_count).as_vector()

        # §8: Perturbation override — force least-recently-used heuristic
        ...
        if self.qtable is not None:
            ...
            # self.tabu is the canonical deque from the base class (preserved)
            tabu_set  = set(self.tabu)

            q_vals    = self.qtable.q_values(state_vec).copy()
            cf_scores = np.array(
                [self._choice_function_score(h, prev_h) for h in range(h_count)],
                dtype=np.float32,
            )

            # Normalise both to [0, 1] before blending so CF recency growth
            # cannot swamp Q-Table signal over time.
            ...
            # BUG FIX 3 (inherited): if all heuristics are tabu, ignore mask
            if np.all(np.isinf(blended)):
                blended = self.q_weight * q_norm + self.cf_weight * cf_norm

            return int(np.argmax(blended))

        # Fallback: parent CF selection (includes OPT 1 tabu set + BUG FIX 3)
    ...
    def _credit_assignment(
        self, h: int, prev_h: Optional[int],
        before: float, after: float,
        accepted: bool, improved: bool,
    ) -> None:
        # Parent CF update (EMA reward, worsening EMA, pair synergy)
        ...
        # §11: Sliding window + dynamic tail window
        ...
        if len(self.sliding_window) >= 10:
            ...
            # Widen tail window when variance is high (unstable search landscape)
            new_win = 20 + int(self.sliding_variance * 60)
            if self.tail_system is not None:
                self.tail_system.set_window(new_win)

        # §10: Online Q-Table update
        # Guard: _pre_action_state must exist (set in _select_heuristic above)
        if self._pre_action_state is not None:
            state_vec      = self._pre_action_state
            next_state_vec = build_state(self, len(self.h_stats)).as_vector()
            ...
            # Online Q update only when a qtable is present
            if self.qtable is not None:
                self.qtable.update(state_vec, h, reward, next_state_vec)

            # Transition hook fires regardless — offline collector needs it
            # even without a qtable attached.
        ...
        # §9: Reset degenerate heuristic rows — if a heuristic is selected
        # frequently but almost never accepted, its Q-row is likely misleading.
        if self.qtable is not None:
            hs = self.h_stats[h]
...
def pretrain_and_deploy(
    ...
    One-liner convenience wrapper::
        ...
        hh = pretrain_and_deploy(training_problems, h_count=8, state_dim=7,
                                 n_pretrain_runs=5)
    ...
    # Q-Table accumulates experience across all pre-training runs
